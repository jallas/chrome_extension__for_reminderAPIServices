...
exports.notification = async (req, res, next) => {
  try {
    ...
    const participants = await models.Participants.query().where({ reminders_id: id });

    if (!participants) throw new Errors.NotFound("Record not found");

    
    //send mail
    participants.forEach(element => {
      const message = `Hi </br></br> ${req.user.user_name} setup a reminder with details below </br></br> ${reminder.details}`;
      sendEmail(element.email, reminder.title, "", message);
    });
